...
const pagesOutput = resolve(root, "docs", "index.html");
...
html = html.replace("<title>", `<meta name="star-color-build" content="single-file-v${version}" />\n    <title>`);
mkdirSync(dirname(output), { recursive: true });
writeFileSync(output, html);
mkdirSync(dirname(pagesOutput), { recursive: true });
writeFileSync(pagesOutput, html);
console.log(`${output}\n${pagesOutput}\n${Buffer.byteLength(html)} bytes each`);
